...
<html @if(isRtl()) dir="rtl" class="rtl no-js" @else class="no-js" @endif lang="zxx">
    ...
    foreach($carts as $cart){
        ...
        if ($cart->product_type === 'product') {
            $price = $cart->price;
        } elseif ($cart->product_type === 'gift_card') {
            $price = $cart->price;
        }
    ...
    $regular_menus = Modules\Menu\Entities\Menu::with('elements.page','elements.childs','elements.childs.page')->where('menu_type', 'normal_menu')->where('menu_position','top_navbar')->whereIn('id',[1,2,16])->orderBy('id')->where('status', 1)->get();
    $topnavbar_left_menu = null;
    $topnavbar_right_menu = null;
    $topnavbar_center_menu = null;
    foreach ($regular_menus as $menu) {
        ...
        elseif ($menu->slug == 'top-navbar-center-menu') {
            $topnavbar_center_menu = $menu;
        }
    }
