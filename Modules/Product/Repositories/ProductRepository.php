        ...
        if (isModuleActive('FrontendMultiLang')) {
            // if (!isModuleActive('MultiVendor')) {
                ...
                else
                {
                    ...
                    $sellerProduct_new->discount = $product->discount;
                    $sellerProduct_new->discount_type = $product->discount_type;
                    $sellerProduct_new->slug = $productSlug;
                    $sellerProduct_new->status = $product->status;
                    $sellerProduct_new->subtitle_1 = $productSubtitle_1;
                    $sellerProduct_new->subtitle_2 = $productSubtitle_2;
                    $sellerProduct_new->save();
                }
               
            // }
        }else{
            // if (!isModuleActive('MultiVendor')) {
                Log::info('multivendor1');
            ...
            if(product_attribute_editable($product->id) === false){
                foreach ($data['track_sku'] as $key => $variant_sku) {
                    ...
                    if ($sku_exist == null) {
                        ...
                        $product_sku->track_sku = $data['track_sku'][$key];
                        $product_sku->selling_price = $data['selling_price_sku'][$key];
                        $product_sku->seller_price = $product->item_id && isset($data['seller_price_sku'][$key]) ? $data['seller_price_sku'][$key] : 0;
                        $product_sku->additional_shipping = isset($data['additional_shipping']) ? $data['additional_shipping'] : 0;
                        $image_increment = $key + 1;
                        $media_img = null;
                        if (isset($data['variant_image_' . $image_increment])) {
                        ...
                        $sku_exist->seller_price = $product->item_id && isset($data['seller_price_sku'][$key]) ? $data['seller_price_sku'][$key] : 0;
...
// Check if the seller product exists before trying to update
        if ($sellerProduct) {
            ...
            $sellerProduct->update([
                ...
                'tax_type' => $product->tax_type,
                'slug' => $productSlug,
                'subtitle_1' => $productSubtitle_1,
                'subtitle_2' => $productSubtitle_2,
                'stock_manage' => $product->stock_manage,
            ]);

            // Now create the SellerProductSKU
        ...
        {
            $sellerProduct_new = new SellerProduct();
            $sellerProduct_new->product_id = $product->id;
            $sellerProduct_new->product_name = $productName;
            $sellerProduct_new->stock_manage = $product->stock_manage;
            $sellerProduct_new->tax = $product->tax;
            $sellerProduct_new->user_id = $product->created_by;
            $sellerProduct_new->tax_type = $product->tax_type;
            $sellerProduct_new->discount =$product->discount;
            $sellerProduct_new->discount_type = $product->discount_type;
            $sellerProduct_new->is_approved = 1;
            $sellerProduct_new->slug = $productSlug;
            $sellerProduct_new->subtitle_1 = $productSubtitle_1;
            $sellerProduct_new->subtitle_2 = $productSubtitle_2;
            $sellerProduct_new->save();
            ...
            foreach ($product_skus as $item) {
                ...
                $sellerProduct_new->update([
                    'min_sell_price' => $sellerProduct_new->skus->min('selling_price'),
                    'max_sell_price' => $sellerProduct_new->skus->max('selling_price')
                ]);
            }
    ...
    public function getSellerProduct(){
        ...
        $exclude_columns = ['description','specification'];
        $get_columns = array_diff($all_columns, $exclude_columns);
        return Product::with('brand')->select($get_columns)->where('user_id', $seller_id);
    }
